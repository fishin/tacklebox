...
/*
   for (var i = 0; i < job.runs.length; i++) {
      ...
      internals.Job.settings.plugins.reel.settings.reel.dirpath = internals.Job.settings.job.dirpath + '/' + job_id + '/runs';
...
exports.getJobByLink = function (link) {
   ...
   var jobPail = new Pail(internals.Job.settings.job);
   var job_id = jobPail.getPailByLink(link);
   var config = internals.Job.getJob(job_id);
   return config;
};
