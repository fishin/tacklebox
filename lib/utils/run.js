...
exports.startRun = function (jobId) {
    ...
    var createConfig = bait.Utils.createRun(commands);
    //console.log('running id: ' + createConfig.id);
    //var runId = internals.Run.settings.plugins.reel.startRun(createConfig.id);
    console.log('runId: ' + runId + ' jobId: ' + jobId);
    var runId = bait.Utils.startRun(createConfig.id);
    var updateJob = job;
...
exports.cancelRun = function (jobId, runId) {
    ...
    var config = bait.Utils.cancelRun(runId);
    return null;
};
...
exports.getRunByLink = function (jobId, link) {
    ...
    var run = bait.Utils.getRunByLink(link);
    return run;
};
