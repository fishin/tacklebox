...
var internals = {
    defaults: {
        apiPath: '/api',
        jobPath: '/tmp/tacklebox/job2'
    }
};

exports.register = function (plugin, options, next) {

    plugin.register(require('reel'), function (err) {
//        if (err) {
...
//        }
    ...
    plugin.route([
        ...
        // { method: 'GET', path: settings.apiPath+'/job/{job_id}/run/{run_id}/cancel', config: { handler: Api.cancelRun, description: "cancel run" } },
        //Parent/Child tests
        // { method: 'POST', path: settings.apiPath+'/children', config: { handler: Api.addChild, description: "add child process" } }        
    ]);
